...
- Uncovered video area: switchable mouse and shooting modes, each with independent sensitivity and
  left-click behavior. The switch can also emit a key combination (for example, Tab to open an
  inventory and enter mouse mode) without affecting the separate aim-area sensitivity.
